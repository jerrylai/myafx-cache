...
public class RedisCache extends BaseCache implements IRedisCache {
    ...
    public RedisCache(String node, String item, RedisConnection redisConnection, ICacheKey cacheKey, String prefix) throws Exception {
        if (node == null || node.isEmpty()) throw new Exception("node is null !"); 
        if (item == null || item.isEmpty()) throw new Exception("item is null !"); 
        if (redisConnection == null) throw new Exception("redis is null !"); 
        if (cacheKey == null) throw new Exception("cacheKey is null !");
        this.keyConfig = cacheKey.get(node, item);
        if(this.keyConfig == null) throw new Exception(node + "/"+item+" 未配置！");
    ...
    protected byte[] serialize(Object value) throws Exception {
        ...
        if(value instanceof byte[] buf) return buf;

        var json = this.mapper.writeValueAsString(value);
    ...
    protected <T> T deserialize(byte[] buffer, Class<T> clazz) throws Exception {
        ...
        var m = this.mapper.readValue(json, clazz);
    ...
    @Override
     public String getCacheKey(Object[] args) throws Exception {
        if (this.keyConfig.Key == null || this.keyConfig.Key.isEmpty()) throw new Exception("cache key(Node="+this.keyConfig.Node+", Item="+this.keyConfig.Item+") is null!");
        var key = this.keyConfig.Key;
        if (args != null && args.length > 0) {
            StringBuilder stringBuilder = new StringBuilder();
            for (int i = 0; i < args.length; i++) {
                var o = args[i];
                if (o instanceof Enum<?> e) o = e.ordinal();
    ...
    public boolean remove(Object[] args) throws Exception {
        ...
        return this.redis.del(getBytes(key)) > 0;
    }
    /**
     * 缓存key是否存在
     * @param args 缓存key参数
    ...
    public boolean contains(Object[] args) throws Exception{
        ...
        return this.redis.exists(getBytes(key));
    }
    /**
     * 设置缓存有效时间
    ...
    public boolean expire(Integer expireSeconds, Object[] args) throws Exception {
        ...
        return expireSeconds != null && expireSeconds > 0 ? this.redis.expire(getBytes(key), expireSeconds) : this.redis.persist(getBytes(key));
    }
    /**
     * 根据系统配置设置缓存有效时间
     * @param args 缓存key参数
