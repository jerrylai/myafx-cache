...
public class LinkListCache<T> extends RedisCache implements ILinkListCache<T> {
    ...
    public long pushLeft(T value, Object... args)throws Exception{
        ...
        var r = this.redis.lPush(cachekeyBytes, serialize(value));
    ...
    public long pushLeft(List<T> list, Object... args) throws Exception {
        ...
        for(int i=0; i<list.size(); i++){
            var v = list.get(i);
            if (v == null) throw new Exception("list item is null!");
            arr[i] = serialize(v);
        }
        var r = this.redis.lPush(cachekeyBytes, arr);
    ...
    public long pushRight(T value, Object... args)throws Exception {
        ...
        this.redis.select(db);
        var cachekeyBytes = getBytes(cachekey);
        var r = this.redis.rPush(cachekeyBytes, serialize(value));
    ...
    public long pushRight(List<T> list, Object... args) throws Exception {
        ...
        var r = this.redis.rPush(cachekeyBytes, arr);
    ...
    public T get(long index, Object... args) throws Exception {
        if (index < 0) throw new Exception("index = "+index+" is error!");
        String cachekey = this.getCacheKey(args);
        int db = this.getCacheDb(cachekey);
        this.redis.select(db);
        var cachekeyBytes = getBytes(cachekey);
        var r = this.redis.lIndex(cachekeyBytes, index);
    ...
    public List<T> getRange(long start, long stop, Object... args) throws Exception {
        if (start < 0) throw new Exception("start = "+start+" is error!");
        if (stop != -1 && stop < start) throw new Exception("stop = "+stop+" is error!");
        String cachekey = this.getCacheKey(args);
        int db = this.getCacheDb(cachekey);
        this.redis.select(db);
        var cachekeyBytes = getBytes(cachekey);
        var r = this.redis.lRange(cachekeyBytes, start, stop);
        List<T> list = null;
        if(r != null){
    ...
    public long insertAfter(T pivot, T value, Object... args) throws Exception {
        if (pivot == null) throw new Exception("pivot is null!");
        if (value == null) throw new Exception("value is null!");
        String cachekey = this.getCacheKey(args);
        int db = this.getCacheDb(cachekey);
        this.redis.select(db);
        var cachekeyBytes = getBytes(cachekey);
        var r = this.redis.lInsert(cachekeyBytes, Position.AFTER, serialize(pivot), serialize(value));
    ...
    public long insertBefore(T pivot, T value, Object... args) throws Exception {
        ...
        this.redis.select(db);
        var cachekeyBytes = getBytes(cachekey);
        var r = this.redis.lInsert(cachekeyBytes, Position.BEFORE, serialize(pivot), serialize(value));
    ...
    public T popLeft( Object... args)throws Exception {
        ...
        this.redis.select(db);
        var cachekeyBytes = getBytes(cachekey);
        var r = this.redis.lPop(cachekeyBytes);
        T m = deserialize(r, clazz);
    ...
    public T popRight(Object... args)throws Exception {
        ...
        this.redis.select(db);
        var cachekeyBytes = getBytes(cachekey);
        var r = this.redis.rPop(cachekeyBytes);
        T m = deserialize(r, clazz);
    ...
    public boolean update(long index, T value, Object... args)throws Exception {
        ...
        this.redis.select(db);
        var cachekeyBytes = getBytes(cachekey);
        this.redis.lSet(cachekeyBytes, index, serialize(value));
    ...
    public long delete(T value, long count, Object... args)throws Exception {
        ...
        this.redis.select(db);
        var cachekeyBytes = getBytes(cachekey);
        var r = this.redis.lRem(cachekeyBytes, count, serialize(value));
    ...
    public void trim(long start, long stop, Object... args)throws Exception {
        if (start < 0) throw new Exception("start="+start+" is error!");
        if (stop < start) throw new Exception("stop="+stop+" is error!");
        String cachekey = this.getCacheKey(args);
        int db = this.getCacheDb(cachekey);
        this.redis.select(db);
        var cachekeyBytes = getBytes(cachekey);
        this.redis.lTrim(cachekeyBytes, start, stop);
    }
    /**
     * 获取链表长度
     * @param args 缓存key参数
    ...
    public long getCount(Object... args)throws Exception {
        ...
        this.redis.select(db);
        var cachekeyBytes = getBytes(cachekey);
        var r = this.redis.lLen(cachekeyBytes);
